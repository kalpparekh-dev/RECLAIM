...
TARGET_RATES = np.array([0.05,0.075,0.10,0.125,0.15,0.20,0.25,0.30], dtype=float)
...
def top_rate_policy(control,treatment,u,rate):
    n=len(u)
    k=max(1,int(np.ceil(n*rate)))
    order=np.argsort(-u,kind='stable')
    p=np.zeros(n,dtype=bool)
    p[order[:k]]=True
    v=np.where(p,treatment,control)
    return float(v.mean()),float(p.mean()),p


def choose_policy_v10_2(u,control,treatment):
    retry=float(np.mean(treatment))
    candidates=[]
    for th in THRESHOLDS:
        v,r,p=policy(control,treatment,u,th)
        candidates.append(('THRESHOLD',float(th),v,r,p))
    for rate in TARGET_RATES:
        v,r,p=top_rate_policy(control,treatment,u,rate)
        candidates.append(('TOP_RATE',float(rate),v,r,p))
    positive=[c for c in candidates if c[3]>0]
    safe=[c for c in positive if c[2]>retry]
    pool=safe if safe else positive
    if not pool:
        raise RuntimeError('No positive-intervention policy candidate available')
...
def run(df,s,cols,control,treatment,seed):
    ...
    selected_kind, selected_parameter, _, _, _ = choose_policy_v10_2(
        uv, control[valid_global], treatment[valid_global]
    )
    _,_,ut=fit_uplift(outer,test,cols,s,seed+20000)
    if selected_kind=='THRESHOLD':
        value,rate,pol=policy(control[te],treatment[te],ut,selected_parameter)
    else:
        value,rate,pol=top_rate_policy(control[te],treatment[te],ut,selected_parameter)
    ...
    return dict(seed=seed,training_rows=len(outer),test_rows=len(test),mean_probability_uplift=float(ut.mean()),selected_policy_type=selected_kind,selected_policy_parameter=float(selected_parameter),selected_threshold=float(selected_parameter) if selected_kind=='THRESHOLD' else np.nan,selected_target_rate=float(selected_parameter) if selected_kind=='TOP_RATE' else np.nan,intervention_rate=rate,targeting_value=value,retry_all_value=retry,no_intervention_value=no,learned_advantage_vs_retry_all=adv,targeting_advantage_vs_no_intervention=value-no,oracle_value=oracle_value,oracle_intervention_rate=float(oracle_pol.mean()),oracle_advantage_vs_retry_all=oadv,qini=q,qini_per_row=q/len(test),oracle_opportunity_capture=adv/oadv if oadv>0 else np.nan,beats_retry_all=adv>0,beats_no_intervention=value>no)


def main():
    header('RECLAIM CAUSAL UPLIFT OPTIMIZER V10.2')
    print('V10 PRESERVED + POSITIVE RATE-AWARE POLICY SELECTION')
    print('V10 ECONOMIC LAYER + QINI + VALIDATION-ONLY POLICY + UNTOUCHED OUTER TEST')
    ...
    for i,seed in enumerate(SEEDS,1):
        ...
        print(f"Selected policy: {r['selected_policy_type']} | parameter: {money(r['selected_threshold']) if r['selected_policy_type']=='THRESHOLD' else f"{r['selected_target_rate']:.2%} TOP_RATE"}")
    ...
    header('V10.2 STABILITY SUMMARY')
    ...
    header('FINAL V10.2 CAUSAL POLICY VERDICT')
    ...
    if qm>0 and al>0 and val.beats_retry_all.mean()>=.80 and val.intervention_rate.mean()>0: verdict='V10.2_TARGETING_SUPPORTED'
    elif al>0 and val.beats_retry_all.mean()>=.80 and val.intervention_rate.mean()>0: verdict='V10.2_ECONOMIC_TARGETING_SUPPORTED'
    elif qm>0 and val.intervention_rate.mean()>0: verdict='V10.2_QINI_AND_INTERVENTION_SUPPORTED'
    else: verdict='V10.2_INCONCLUSIVE'
    ...
    pt,pc,u=fit_uplift(df,df,cols,s,123456)
    kind_counts=val.selected_policy_type.value_counts()
    final_kind=kind_counts.index[0]
    final_parameter=float(val.loc[val.selected_policy_type==final_kind,'selected_policy_parameter'].median())
    if final_kind=='THRESHOLD':
        fv,final_rate,fp=policy(control,treatment,u,final_parameter)
        print('Median selected threshold:',money(final_parameter))
    else:
        fv,final_rate,fp=top_rate_policy(control,treatment,u,final_parameter)
        print('Median selected target rate:',f'{final_parameter:.2%}')
    fr=treatment.mean()
    print('Final policy type:',final_kind)
    print('Full-data intervention rate:',f'{final_rate:.2%}')
    print('Full-data targeting value:',money(fv))
    print('Full-data RETRY_ALL value:',money(fr))
    print('Full-data advantage:',money(fv-fr))
    pred=pd.DataFrame(index=df.index)
    for c in ['transaction_id','customer_id']:
        if c in df.columns: pred[c]=df[c]
    pred['predicted_treatment_probability']=pt; pred['predicted_control_probability']=pc; pred['predicted_probability_uplift']=u; pred['selected_policy_type']=final_kind; pred['selected_policy_parameter']=final_parameter; pred['selected_policy']=np.where(fp,'TARGET','RETRY_ALL'); pred['intervention_selected']=fp.astype(int)
    ...
    summary=pd.DataFrame([{'version':'V10.2','validation_runs':N_RUNS,'mean_qini':qm,'qini_ci_lower':ql,'qini_ci_upper':qh,'positive_qini_rate':(val.qini>0).mean(),'mean_probability_uplift':val.mean_probability_uplift.mean(),'mean_intervention_rate':val.intervention_rate.mean(),'mean_targeting_value':val.targeting_value.mean(),'mean_retry_all_value':val.retry_all_value.mean(),'mean_learned_advantage':am,'learned_ci_lower':al,'learned_ci_upper':ah,'runs_beating_retry_all':val.beats_retry_all.mean(),'mean_oracle_advantage':om,'oracle_ci_lower':ol,'oracle_ci_upper':oh,'mean_oracle_opportunity_capture':val.oracle_opportunity_capture.mean(),'final_policy_type':final_kind,'final_policy_parameter':final_parameter,'full_data_intervention_rate':final_rate,'full_data_targeting_value':fv,'full_data_retry_all_value':fr,'full_data_advantage_vs_retry_all':fv-fr,'final_verdict':verdict}])
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True)
    paths={'Validation':OUTPUT_DIR/'causal_uplift_v10_2_validation_results.csv','Predictions':OUTPUT_DIR/'causal_uplift_v10_2_predictions.csv','Policies':OUTPUT_DIR/'causal_uplift_v10_2_policy_comparison.csv','Summary':OUTPUT_DIR/'causal_uplift_v10_2_summary.csv'}
    ...
    header('V10.2 COMPLETE'); print(f'Mean QINI: {qm:,.2f}'); print('Positive QINI rate:',f'{(val.qini>0).mean():.2%}'); print('Mean learned advantage:',money(am)); print(f'Learned 95% CI: [{money(al)}, {money(ah)}]'); print('Mean oracle advantage:',money(om)); print('FINAL VERDICT:',verdict)
